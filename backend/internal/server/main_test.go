...
func addTestTweet(user *store.User) *store.Tweet {
	post := &store.Tweet{
		Content: "Joker is planning a big hit tonight.",
	}
	err := store.AddTweet(user, post)
	if err != nil {
...
func addTestTweet2(user *store.User) *store.Tweet {
	post := &store.Tweet{
...
func postJSON(post store.Tweet) string {
	body, err := json.Marshal(map[string]interface{}{
		"ID":      post.ID,
